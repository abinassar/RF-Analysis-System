...
export class GlobalMapComponent implements OnDestroy, OnChanges {
  ...
  @Input() P1: GeoPoint;
  @Input() P2: GeoPoint;
  @Output() sendP1 = new EventEmitter<GeoPoint>();
  @Output() sendP2 = new EventEmitter<GeoPoint>();
  ...
  updateGeoPoints(markers: any[]) {
    ...
    markers.forEach((marker, index) => {
      ...
      if (index === 0) {
        ...
        this.sendP1.emit(point);

      } else {

        this.settingsService.linkSettings.P2 = point;
        this.sendP2.emit(point);
  ...
  ngOnDestroy(): void {
    ...
    this.refreshMapObservable.unsubscribe();
  }
